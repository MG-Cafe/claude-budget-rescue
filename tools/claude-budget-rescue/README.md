...
If the plugin companion is not ready, the tool falls back to a background `codex exec` run. After a successful automatic launch, check the background Codex job from the same terminal:
